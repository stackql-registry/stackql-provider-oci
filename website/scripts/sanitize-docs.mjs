...
//
// Generated OCI descriptions can carry literal angle-bracket placeholders
// (<region>, <compartment-ocid>), stray unpaired HTML (</code>, <p>) and
// XML samples. MDX v3 parses any raw <token> as JSX and
...
function escapeDescription(inner) {
  ...
  out = out
    ...
    .replace(/\{/g, '&#123;')
    .replace(/\}/g, '&#125;')
    // Regex fragments in descriptions ("oci://([^/]+)(/.*)?") read as
...
// source (e.g. descriptions carrying an already-escaped literal "\${...}").
...
function sanitize(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // sequences like backslash-u007F in description text become raw control
